...
public class QuestionListFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        doneFlag = getArguments().getBoolean("Done");
        gridView = view.findViewById(R.id.gridView);

        // 파이어베이스에서 데이터 삽입,받아오기
        mDatabase = FirebaseDatabase.getInstance().getReference("QuestionInfo");
        /*String newquestion = mDatabase.push().getKey();
        QuestionInfo questionInfo = new QuestionInfo(newquestion,"images/20181118_0809.png","a",true);
        mDatabase.child(newquestion).setValue(questionInfo);*/
        getFirebaseData();

        // 홈 버튼 눌렀을 때
        getActivity().findViewById(R.id.button_home).setOnClickListener(new View.OnClickListener() {
            @Override
       ...
       mDatabase.limitToLast(10).orderByChild("q_dataTime").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for(DataSnapshot snapshot : dataSnapshot.getChildren()) {
                    ...
                    if((!doneFlag) && (!questionInfo.checkAnswer))
                        imagePath.add(questionInfo);
                    else if(doneFlag && questionInfo.checkAnswer)
                        imagePath.add(questionInfo);
                }
                Collections.reverse(imagePath);
                // 그리드 뷰 사용
                gridImageAdapter = new QuestionListImageAdapter(context, imagePath, doneFlag);
